...
Three unrelated bugs found through real daily use of the app, all with genuine root causes rather than surface patches. No schema or backend changes — `index.html` only.

### Home page badges backwards on the last day of the week

`getHomeMetricBadge()` (renderHomeHero's per-metric badge) judges over/under target via a "pace" value from `getWeeklyRequiredDaily()`, deliberately favouring the daily rate still *needed* for the rest of the week over the raw average so far — an early-week dip that's still recoverable shouldn't read as "falling behind." While days remain in the week, needing *more* than target for the rest of the week (`paceValue > target`) always corresponds to a real current deficit (`avg < target`), and needing *less* (`paceValue < target`) always corresponds to a real current surplus — provably the same sign as a plain avg-vs-target comparison for any positive number of days remaining, just scaled by how much time is left to react.
...
The entire app — every screen, modal, and gate — lives in one `index.html` with **no `<form>` element anywhere** in the whole document; every input has always just been a bare `<input>` inside plain `<div>`s. This was harmless until v8.06 (this same session, earlier) added the Change Password modal, which put two more real `type="password"` inputs onto that same unwrapped page (on top of the pre-existing sign-in password field and the Settings → API key field, also `type="password"` purely to mask the key visually). With no `<form>` boundary anywhere to scope them, iOS Safari's autofill heuristics can't tell any of these password fields are limited to a specific flow — once enough of them exist on one page, Safari starts offering its Passwords/Keychain AutoFill bar for *any* input on the page it judges plausible, including the plain numeric Log Steps box on Home, replacing the numeric keypad with a passcode-style keyboard.

**Fix:** wrapped each of the three password-bearing sections in its own `<form style="display:contents" onsubmit="return false;">` — display:contents means the form contributes no box of its own, so it's invisible to every existing flex/grid layout around it (confirmed pixel-identical via Playwright screenshot comparison before/after) — giving Safari a real scope boundary per flow: the auth-gate sign-in form, the Change Password form, and the Settings API-key form. Also added `autocomplete="off"` to the API-key field itself, since it was the one password-type field with no autocomplete hint at all. No other inputs on the page were touched.

### Splash screen flashed the sign-in gate before playing, even for a signed-in return visit

`#auth-gate` is deliberately fail-closed by design — visible by default in the markup, with JS only ever adding a `.hidden` class once a session is positively confirmed — and sits at `z-index: 10000`, above the splash's `9999`, so a genuinely signed-out person is never shown the app or its splash animation underneath the gate. The splash itself starts playing immediately on `DOMContentLoaded`, entirely independent of auth state.

The bug: `onAuthResolved()` — the function that hides the gate once a session is confirmed — is only ever called from `initSupabaseAuth()`'s `await supabase.auth.getSession()`, which returns a `Promise` even when it's just reading a session already cached in `localStorage`. There is no synchronous path to hiding the gate, however fast the underlying check is, so on every load the gate is what's actually painted first, however briefly, before JS has had a chance to run and hide it — including for an already-signed-in return visit. A prior comment on `#auth-gate`'s CSS block claimed this "resolves fast enough... that the gate is already hidden well before the splash even finishes, so no flash either way" — true of the *splash*, but irrelevant to the *first paint*, which is what the person actually sees flash.

**Fix:** added a synchronous inline `<script>` immediately after `<body>` — before `#splash` or `#auth-gate` are even parsed — that checks `localStorage` for a key matching supabase-js's own persisted-session pattern (`/^sb-.*-auth-token$/`, deliberately not hardcoding the project ref, so this keeps working if `SUPABASE_URL` ever changes) and, if found, adds `auth-likely-signed-in` to `<html>`. A new CSS rule (`html.auth-likely-signed-in #auth-gate { display: none; }`) hides the gate immediately, before any script tied to the real auth flow has even executed, let alone resolved.

This is **optimistic, not authoritative** — presence of a cached token doesn't prove the session is still valid, so the real async check in `initSupabaseAuth()`/`onAuthResolved()` still runs exactly as before and is still what actually gates the app:
- Session confirmed valid → `hideAuthGate()` runs as before; harmless no-op on top of a gate that was already invisible.
- Session comes back invalid/stale, or the check fails outright (network error, `showAuthGateError()`) → `showAuthGate()` now also does `document.documentElement.classList.remove('auth-likely-signed-in')` before removing `.hidden`, so the real gate correctly reappears. Worst case is a rare corrective pop-in for a stale token, not a flash on every load.
...
`renderProgress()`'s Card 3 now computes `nextCyclePlanChosen` — true when not previewing a different macro AND `state.nextCycleAdvice.macroId === macro.id` AND `state.nextCycleAdvice.chosenPlanKey` is set. When true, the entire deterministic block (`headerHtml`, `continuationAlternativeHtml`, `directionChoiceHtml`, `maintenanceInfoHtml`, `targetInputHtml`, `bridgeTableHtml`, `directionRampTableHtml`, `buildButtonHtml`) is suppressed, leaving only BLOC's advice section (which shows the chosen plan and its steps) visible.

**Known trade-off, accepted as-is:** if someone chooses a BLOC plan (tapping "Use [Plan] →") but never actually completes the "Create Macrocycle" save afterward, the deterministic recommendation stays hidden indefinitely even though nothing was built — `chosenPlanKey` persists once set, with no way to un-choose it from the UI today. Worth a follow-up if this turns out to happen in practice; not addressed this session.

**Still open:** the pure-deterministic path (tapping "Build this plan next" directly, without ever going through BLOC's advice — no `state.nextCycleAdvice` involved at all) isn't covered by this fix, since `nextCyclePlanChosen` requires a chosen BLOC plan to be true. That path still relies on §68's `progressViewMacroId` fix alone, whose real-world reliability wasn't fully confirmed this session. Needs a dedicated retest: build a cycle via the deterministic button only (skip "Ask BLOC for a second opinion" entirely) and check whether the button reappears.

## §71 — v8.12: Final-week card — Build next cycle + Cycle review combined, with sequencing, refresh, and a no-op guard

Request: `bloc-app/2026-09-15/dev 2026-09-15.md` item 1. During a cycle's final calendar week, the mid-cycle check-in card is replaced by a single card with two independent sections — Build next cycle (BLOC's own second opinion on what comes after) and Cycle review (the existing one-time, per-cycle review) — rather than the deterministic-only "Next cycle" card that used to live in its own swipe slide (see §74, same session).

### Two-section shell

`isInFinalWeek(macro)` is true for the 7 days up to and including a macro's actual last day (`getMacroEndDate()` minus 6 days through that date). `buildFinalWeekCardHTML(macro, rec)` takes over `renderProgress()`'s card-1 slot whenever this is true **and** `macro.id === state.currentMacroId` — the active-cycle check matters because `isInFinalWeek()` has no upper bound, so without it a long-past, never-reviewed macro browsed via the hero selector would show "Build next cycle" forever too, not just Cycle review (caught during this session's own tour-update testing, fixed same day — a past cycle now falls through to the unchanged `buildCycleReviewCardHTML()` plain view instead, reachable indefinitely as before).

- **Build next cycle** reuses the existing Phase 4 second-opinion feature (`buildNextCycleAdviceSectionHTML()`/`askBlocForNextCycleAdvice()`, §-numbered in earlier sections) essentially verbatim — its own loading/CTA/narrative/plan-choice states already covered everything needed; only the CTA button's label changed, from "Ask BLOC for a second opinion" to "Build next cycle", to match today's terminology. Available for the whole final week.
- **Cycle review** shows a countdown pill until the actual last day, then "Start Cycle Review" (`openCycleReviewModal()`, unchanged). `buildCycleReviewSummaryHTML(macro)` was extracted out of `buildCycleReviewCardHTML()` so the score header + collapsible narrative/highlights/improvements markup is shared rather than duplicated — this final-week card keeps showing both sections (embedding the completed review inline, keyed by the same `_cycleReviewCardOpen`/`toggleCycleReviewCard()` state) for as long as the cycle stays active, rather than switching to `buildCycleReviewCardHTML()`'s plain whole-card view the moment the review completes.

### Sequencing (both API calls when needed)

If Build next cycle is clicked before Cycle review has run — and review is actually due (the last day has arrived) — `startBuildNextCycleFlow()` runs the review first, silently (no photos, since its modal was never opened), via a new `generateCycleReview(macro, beforeImages, afterImages)` extracted out of `runCycleReview()` (the core API-call+validation+`macro.review`-write, with no DOM/modal coupling). `buildCycleReviewPrompt()` now takes before/after images as explicit parameters instead of reading the modal's global photo buffers directly, so it works correctly from either caller. Both sections show "BLOC is thinking…" simultaneously the moment either button is clicked (`_cycleReviewLoading`/`_nextCycleAdviceLoading` both set together during the review phase); each section's content still renders independently the instant its own response arrives.

`buildNextCycleAdvicePrompt()`'s user message now includes a CYCLE REVIEW section (compliance score, bodyfat estimate, headline, sticking points) whenever `macro.review` exists by the time it's built — covering both orderings (review-first, or auto-sequenced) with one code path, since the prompt doesn't need to know which happened.

Generalized `computeCycleReviewPayload()`'s missing-final-day-data handling from steps-only to kcal/protein/carbs/steps/weight alike: any of these missing on the cycle's actual last day gets substituted with that week's average-so-far and a note telling the LLM to treat it as met rather than a miss or data gap (the averages themselves are unaffected — they already exclude missing days, not zero-fill them, so this is purely about telling the model not to penalize a data-entry gap it can't otherwise distinguish from a real miss).

### Refresh with cycle review

If Build next cycle ran *before* Cycle review was even due, its response never saw the review's verdict. Once the review completes, a one-shot "Refresh with cycle review →" link appears (styled/behaved like "Challenge this advice" for check-ins — spent on click), only while no plan has been chosen yet. Refreshing re-sends the same prompt with the original response and the review both included, via a new optional `priorResponse` parameter on `buildNextCycleAdvicePrompt()`/`askBlocForNextCycleAdvice()`, instructed to adjust only if the review genuinely changes something. A new `reviewIncluded` flag on `state.nextCycleAdvice` (true whenever `macro.review` existed at storage time, refresh or not) drives the one-shot gating — doubles as "was the review folded in" and "so refresh is no longer offered."

### Prompt: never reference the deterministic recommendation

The build-next-cycle prompt previously told the model to explicitly agree or disagree with "the deterministic recommendation's own reasoning" in its narrative — backwards from what was wanted. The deterministic engine's output still reaches the model as input context (pacing/safety boundaries, the continuation/conflict framing), but a new HARD CONSTRAINT plus a rewritten `narrative` schema description now require the response to read as an independent assessment, with no reference to "the app's suggestion," "the deterministic engine," or "confirming"/"agreeing with" a prior recommendation anywhere in the output.

### No-op guard

New `planMatchesActiveGoal(plan)`: true when a plan's first (immediately-actionable) step is numerically identical to the currently active goal on kcal/protein/carbs/steps only. Applied per-plan (not per-response) at all 4 "Use/Switch to [plan]" button render sites — 2 in the mid-cycle check-in card (`buildAiAdviceCardHTML`, pre-choice and post-choice-alternative), 2 in the build-next-cycle card (`buildNextCycleAdviceSectionHTML`, same split) — replacing the button with a short "No change from your current goal — nothing to build" note when it matches, while the plan's numbers/narrative stay visible either way. This automatically covers the post-"Challenge this advice"-accept case too, since `acceptBlocChallenge()` re-renders through the same check-in `planSection`.

### Local dev tooling (this session only, not itself a product feature)

Testing all of the above required reaching a real cycle's final week/last day without waiting weeks for one — added a local-only auth bypass (`IS_LOCAL_DEV`, gated to `localhost`/`127.0.0.1`, impossible on the real GitHub Pages deploy) that skips Supabase/OAuth entirely and seeds the Demo Tour dataset silently (no tour walkthrough) via `enterDemoMode()`. An optional untracked `bloc-demo-data.dev.json` (gitignored, falls back to the real tracked `bloc-demo-data.json` when absent) carries a `_devAnchorDate` override applied via the existing `setTourAnchorDate()` mechanism. Also discovered and fixed in passing: `enterDemoMode()` swapped `state` directly and skipped every migration `load()` normally applies (extracted into a shared `ensureStateDefaults()`, now called from both) — without it, `state.nextCycleAdviceHistory` being undefined would have crashed the real Demo Tour too, once it was updated (§75) to actually reach this feature.

## §72 — v8.12: Hide Best-7-day-window / Calorie target / Goal weight ETA for maintenance cycles

Request: dev 2026-09-15 item 3. None of these three Progress-page metrics have meaning during a maintenance bridge — no directional "best week," no deficit/surplus target, no target weight to project an ETA toward. Gated all three on `macro.goalType !== 'maintenance'`. Also fixes a pre-existing bug in the calorie-target line: a maintenance cycle previously fell through to the deficit-target wording by accident, since `isGainCycle` (`goalType === 'gain' || 'strength'`) is false for `'maintenance'` too — it was silently showing an incorrect cut-style target rather than nothing.

## §73 — v8.12: Cycle-review photo upload — single tap-target boxes, no emoji

Request: dev 2026-09-15 item 2. The before/after boxes in `#modal-cycle-review` are now the click target themselves (dashed border, "Tap to add photos"), opening the file picker directly, instead of two separate Take/Upload buttons underneath each box. A plain `<input type=file accept=image/*>` with no `capture` attribute already surfaces the OS's camera/library picker sheet on mobile, so dropping the camera-only input loses nothing. Removed the now-redundant camera-only inputs and every emoji icon from this modal.

## §74 — v8.12: Removed the deterministic "Next cycle" card from the Progress swipe deck

Request: dev 2026-09-15 item 4. The Progress page's insights swipe deck goes from 3 slides to 2 (Progress, Metabolism) — the old "Next cycle — weekly kcal projections" card (deterministic recommendation header, direction-choice buttons, target/deadline inputs, ramp tables, the old "Build this plan next" button, and the LLM second-opinion section that used to render below it) is no longer part of the deck at all. `recommendNextCycle()`'s output stays computed exactly as before — it still feeds the build-next-cycle AI prompt (§71) — only this card's own UI is gone. The LLM second-opinion feature that used to live at the bottom of this card now lives in the final-week card instead (§71); the deterministic engine's raw recommendation UI (direction choice, target/deadline override inputs, ramp tables) has no replacement — it's simply retired, per the request that the deterministic engine "stays behind" purely to feed the prompt going forward.

## §75 — v8.12: Demo tour — fixed a step broken by §74, added final-week showcase

§74's card removal broke an existing Demo Tour step (and the identical step in the re-triggerable Progress mini-tour) that jumped `insightsIndex` to the now-nonexistent 3rd slide — it was rendering the literal text "undefined" into the card instead of real content.

**Demo Tour** (`buildDemoTourSteps()`, runs against the demo dataset): replaced the single broken step with three — an intro step that jumps the tour's simulated "today" forward to the same macro's actual last day (2026-09-13), then one step each for the final-week card's two sections. The date anchor resets back to the dataset's real date (2026-08-02) in the following `progress-tables-wrap` step, before Plan/Train/Nutrition — those steps depend on the original date for their own week/day context and must not see the advanced one leak forward.
